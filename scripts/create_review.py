...
<p>展开每一项可查看依据，点击时间定位到短片中的动作。助手初标来自按教学规则查看视频的过程，尚未经过教练确认；本页展示已有记录。</p>
<p>试用辅助手自动候选：在项目文件夹双击 <code>start-coach.cmd</code>，再选择一段本机短片，确认持拍手和练习场景。首次使用请先运行 <code>setup-auto.cmd</code>。候选用于慢放复核，不会自动写入教练标签。</p>
